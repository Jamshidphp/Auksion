const ADMINS = [] // ID Администрации бота
const DB_URL = '' // База данных бота [URL] Данный формат ссылки подходит для серверов, если у вас другой - не бойтесь менять его!
const QIWI_TOKEN = "" // Токен QIWI [Все галочки, если не нужны авто выводы - убираем "Переводы без смс"]
const BOT_TOKEN = "" // Токен бота [@Botfather]
var txnId = require('./txnId');
var config = {
	payeer: {
		enabled: true, // True - Включен, False - Выключен
		account: "P123456789", // Номер PAYEER кошелька
		apiId: 123456789, // ID Токена PAYEER
		apiPass: "Ключ" // Ключ Токена PAYEER
	}
}

const auctionChannel = "@TGAuksionlar" // Юзер канала аукциона

process.env.TZ = 'Moscow/Europe'; // Персонажи нам тут не нужны
let trees = [
	{
		id: 0,
		name: "Коржик",
...
var wbProfits = [0, 33, 340, 618, 982, 2200, 4978, 7470, 23027] // не нужно
var wbPrices = [0, 0, 120, 400, 500, 2000, 3000, 4000, 7000] // не нужно
...
const Start = [ // Кнопки бота [Если собираетесь их переименовывать, переименоввайте их и в ответах!]
	["👨‍⚖ Аукцион", "📱 Профиль"], 
	["👥 Друзья", "📊 Инфо"]
];

const Cancel = [ // Меню отмены
	["🚫 Отмена"]
];

const AdminPanel = [ // Не работающая админка
	["📬 Рассылка", "📮 Выплаты"],
	["📧 Информация"],
	["🔙 Назад"]
];

const RM_admin = { // Админка
	inline_keyboard: [
			...
			if (!$user) {
				...
				if (reffer) {
					...
					if ($reffer) {
						...
						await $reffer.inc('buybalance', 0.50); // Награда за реферала
						await $reffer.inc('refCount', 1); // Не трогать
						bot.sendMessage($reffer.id, `👥 Вы пригласили <a href="tg://user?id=${message.from.id}">друга</a> и получили 0.50₽`, { parse_mode: "HTML" }); // Сообщение после создания промокода
					}
				}
			...
			if (postfix) {
				if (postfix[0] == "C") {
					...
					if (c) {
						...
						bot.sendPhoto("@", `a${c.tree_id}.jpg`,  { caption: `✅ <a href="tg://user?id=${message.chat.id}">Пользователь</a> активировал чек и получил Котёнка - <b>${(trees.find((a) => a.id == c.tree_id)).name}</b>`, parse_mode: "HTML" });

						return bot.sendPhoto(message.chat.id, `a${c.tree_id}.jpg`, { caption: `✅ Вы успешно активировали чек и получили Котёнка - <b>${(trees.find((a) => a.id == c.tree_id)).name}</b>`, parse_mode: "HTML", })
					}
				}
			} // Сообщение после команды /start
		
				return message.send(`
👤 <b>Приветствуем вас, ${message.from.first_name}
👨‍⚖️ Это Лучший Бот - Аукцион в Telegram
💳 Пополняй счёт и вперёд к победе!</b>`, {
				parse_mode: "HTML",
				reply_markup: {
					keyboard: $menu,
	...
	} // Обязательные подписки, меняем каналы как тут, так и в ссылках и ставим бота администратором каналов со всеми правами, если это чат - то все правами кроме анонимности!
	
		if ((await bot.getChatMember("@Выплаты", uid)).status == "left" || (await bot.getChatMember("@Чат", uid)).status == "left" || (await bot.getChatMember("@Спонсор", uid)).status == "left" || (await bot.getChatMember("@Wistienedoprog", uid)).status == "left") {

		return message.send(`👋 <b>Привет, для работы бота пожалуйста подпишись на все каналы ниже!</b>`, { parse_mode: "html", reply_markup: { inline_keyboard: [[{ text: "💳 Выплаты", url: "https://t.me/" }], [{ text: "💬 Чат", url: "https://t.me/"}], [{ text: "💳 Спонсор", url: "https://t.me/"}], [{ text: "🏪 Лучший Магазин Ботов", url: "https://t.me/Wistienedoprog"}] ] } });
}
	...
	if (state[uid] == 77771 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		return message.send(`${text} Котят выпущено на продажу по цене ${bm.price}!`, { reply_markup: { inline_keyboard: [[{ text: "◀️ Назад", callback_data: "admin_bm" }]] } });
	}
	if (state[uid] == 77772 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		await User.findOneAndUpdate({ id: 1 }, { menu: JSON.stringify(bm) })
		return message.send(`Стоимость Котёнка установлена в ${bm.price} рублей!`, { reply_markup: { inline_keyboard: [[{ text: "◀️ Назад", callback_data: "admin_bm" }]] } });
	}
	if (state[uid] == 77773 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		bm.count = Number(text)
		await User.findOneAndUpdate({ id: 1 }, { menu: JSON.stringify(bm) })
		return message.send(`Количество доступных Котят установлено в ${bm.count} штук!`, { reply_markup: { inline_keyboard: [[{ text: "◀️ Назад", callback_data: "admin_bm" }]] } });
	}
	...
	if (state[uid] == 7771 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		bot.sendMessage("@Выплаты", `💳 <a href="tg://user?id=${message.chat.id}">Администратор</a> установил бонус к пополнению в <b>${text}%</b>`,{parse_mode: "HTML" })
	...
	if (state[uid] == 77712 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		bot.sendMessage("@Выплаты", `♻️ <a href="tg://user?id=${message.chat.id}">Администратор</a> установил бонус к реинвесту в <b>${text}%</b>`,{parse_mode: "HTML" })
	...
	if (state[uid] == 7771222 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		text = Number(text.replace("%", ""))
		await User.findOneAndUpdate({ id: 0 }, { bhivebalance: text })
		return message.send(`Лимит на Котят в ${text} единиц установлен!`, { reply_markup: RM_admin_return });
	}
...
▪️ Ваш профит: <b>${sum * 1.5}₽</b>`, {
		...
		bot.sendMessage("@", `🤑 <a href="tg://user?id=${message.chat.id}">Администратор</a> установил бонус к выводу в <b>${text}%</b>`,{parse_mode: "HTML" })
	...
	if (state[uid] == 7772 && ADMINS.indexOf(message.from.id) !== -1 && text != "0") {
		...
		let u = user
		if (!user) return message.send('Пользователь не найден'); // Сообщение при ненахождении пользователя

		let partners = await User.find({ ref: message.text });
		await message.user.set('menu', '');
		var kb = { inline_keyboard: [] } // Админ меню, управление пользователями
		...
		kb.inline_keyboard.push([{ text: "➕ Баланс игр", callback_data: "addBuyBal_" + u.id }, { text: "✏️ Баланс игр", callback_data: "editBuyBal_" + u.id }])
		...
		/*kb.inline_keyboard.push([{ text: "❌ Забрать Котят", callback_data: "takeTree_" + u.id }, { text: "Обнулить пользователя", callback_data: "adminwipe_" + u.id }])*/
...
🎲 Баланс игр: ${user.buybalance.toFixed(2)}₽
💳 Баланс вывода: ${user.outbalance.toFixed(2)}₽
...
`, {
	...
	if (state[uid] == 7773 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		bot.sendMessage(data[uid], `🎲 Ваш баланс игр пополнен на <b>${text}₽</b>!`, { parse_mode: html })
	...
	if (state[uid] == 7774 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		bot.sendMessage(data[uid], `💳 Ваш баланс вывода пополнен на <b>${text}₽</b>!`, { parse_mode: html })
	...
	if (state[uid] == 777455 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		bot.sendMessage(data[uid], `📥 Сумма ваших пополнений пополнена на <b>${text}₽</b>!`, { parse_mode: html })
	...
	if (state[uid] == 77745555 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		bot.sendMessage(data[uid], `📤 Сумма ваших выводов пополнена на <b>${text}₽</b>!`, { parse_mode: html })
	...
	if (state[uid] == 7775 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		bot.sendMessage(data[uid], `🎲 Ваш баланс игр изменён на <b>${text}₽</b>!`, { parse_mode: html })
  ...
  bot.sendMessage(data[uid], `${text}`, { parse_mode: html })
	...
	if (state[uid] == 7776 && ADMINS.indexOf(message.from.id) !== -1) {
		...
		bot.sendMessage(data[uid], `💳 Ваш баланс вывода изменён на <b>${text}₽</b>!`, { parse_mode: html })
	...
	if (state[uid] == 777655 && ADMINS.indexOf(message.from.id) !== -1) {
		state[uid] = undefined
		await User.findOneAndUpdate({ id: data[uid] }, { deposit: Number(text) })
		bot.sendMessage(data[uid], `📥 Сумма ваших пополнений измена на <b>${text}₽</b>!`, { parse_mode: html })
	...
	if (state[uid] == 77765555 && ADMINS.indexOf(message.from.id) !== -1) {
		state[uid] = undefined
		await User.findOneAndUpdate({ id: data[uid] }, { payout: Number(text) })
		bot.sendMessage(data[uid], `📤 Сумма ваших выводов измена на <b>${text}₽</b>!`, { parse_mode: html })
	...
	if (state[uid] == 77777) {
		...
		if (message.text < 10) return message.send('❗️ Минимальная начальная ставка - 10 рублей!')
		if (message.text > message.user.buybalance) return message.send('❌ На башем балансе недостаточно средств, введите другую сумму:')
		var a = await getAuction()
		if (a.status == true) return message.send('❗️ Аукцион уже начался!')
	...
	if (state[uid] == 66667) {
		...
		if (message.text <= message.user.game_balance) {
			...
			state[uid] = undefined
			return message.send(`💳 Вы успешно вывели ${message.text}₽ с игрового баланса на баланс вывода!`, {
				reply_markup: {
					keyboard: $menu,
					resize_keyboard: true
				}
	...
	if (message.user.menu.startsWith('amountQiwi')) {
		...
		if (message.text > message.user.outbalance) return message.send('❌ Недостаточно средств.');
		if (message.text < 9) return message.send('❌ Введите сумму более 9 рублей!');
		...
		if (message.text <= message.user.outbalance) {
			...
			await message.user.set('menu', '');
			// Сообщение админу
			ADMINS.map((x) => bot.sendMessage(x, "💳 Вам пришла заявка на вывод. Перейдите в Админ панель для её одобрения!"));
			// Рассылка в канал
			bot.sendMessage("@Выплаты", `💳 <a href="tg://user?id=${message.chat.id}">Пользователь</a> создал новую заявку на вывод !`,{parse_mode: "HTML" })

			return message.send('✅ Заявка на выплату создана, Админипстрации уже пришло уведомление о её создании! ❗️ Пожалуйста не пишите Администрации по поводу сроках прихода выплат и т.п. - в данном случае вы можете не получить свой вывод!', {
				reply_markup: {
					keyboard: $menu,
					resize_keyboard: true
				}
	...
	if (message.user.menu == 'qiwi') { // 
		...
		if (message.text.length < 0) return message.send('Введите правильный номер!', {
			reply_markup: {
				keyboard: Cancel,
				resize_keyboard: true
			}
	...
	if (message.text) {
		if (message.text == '🐈 отяегапрота') {
			return message.send('Выберите, куда зайти.', {
				reply_markup: {
					inline_keyboard: [
						[{ text: "🛒 Магазин", callback_data: "store" }, { text: "🐈 Мои Котята", callback_data: "trees:inv0" }],
						[{text: "", callback_data: "promoact"}]
					]
				}
			});
		}
		if (message.text == '🎲 Тампапвагочи') {
			var reply_markup
			var t = await Tamagochi.findOne({ creator_id: uid })
			if (!t)
				reply_markup = { inline_keyboard: [[{ text: "🧱 Купить героя за 300 рублей", callback_data: "malBuy" }]] }
			else
...
<b>Стоимость 300 рублей</b>
						`, {
				reply_markup, parse_mode: "html"
			});
		}

		if (message.text == '🎲 вапвапапвапвваппаапапавИры') {
		...
		if (message.text == '♻️вапвапвап Обмен') {
			return message.send(`Здесь Вы сможете обменять <b> 🧶  Защиту</b> на <b>₽ рубли</b>
==================
1000  Клубков 🧶 = 1 рубль
Минимальная сумма обмена: 1000  🧶  Coins
==================
 🧶 <b>Ваши Coins: ${message.user.fetuses.toFixed(2)}
==================								
После обмена 50% попадает на баланс для покупок, а остальный 50% на баланс для вывода:</b>`, {
				reply_markup: {
					inline_keyboard: [
		...
		if (message.text == '📱 Профиль') {
			return message.send(`🪪 Ваше имя: <b>${message.from.first_name.replace(/(\<|\>)/g, '')}</b>
🆔 <b>ID:</b> <code>${message.from.id}</code>
==========================
🎲 <b>Баланс игр:</b> ${message.user.buybalance.toFixed(2)}₽
💳 <b>Баланс вывода:</b> ${message.user.outbalance.toFixed(2)}₽
==========================
...
📤 <b>Выведено:</b> ${message.user.payout.toFixed(2)}₽`, {
				...
				reply_markup: {
					inline_keyboard: [
						...
						[{ text: "🎟 Промокод", callback_data: "promoact" }, { text: "💱 Реинвест", callback_data: "reinvest" }],
					]
				}
			});
		}
...
, {
    parse_mode: "HTML",
       reply_markup: {
 		...
 		if (message.text == '👨‍⚖ Аукцион') {
			return message.send(`👨‍⚖️ <b>Правила аукциона:
...
⚜️ На момент завершения аукциона, победитель получает 80% суммы от всех ставок аукциона
⚜️ Если ставка единственная (никто не перебил стартовую ставку) аукцион завершится через 12 часов, открывшему начисляется 100% обратно!

👨‍⚖ Пусть победит сильнейший! </b>`, {
				parse_mode: "HTML",

reply_markup: {
						inline_keyboard: [
							[{ text: "👨‍⚖️ Аукцион", url: "https://t.me/Аукцион" }],
					]
				}
			});
		}
 
 if (message.text == '🤫 Клвмпмпмпмвпмад') {
        ...
        if (state[uid] == 11000) {
			let postfix = message.text
				if (postfix[0] == "P") {
					...
					var sum = c.sum
					if (c) { // Промики
						bot.sendMessage("@Выплаты", `🎟 <a href="tg://user?id=${message.chat.id}">Пользователь</a> активировал промокод для покупок на сумму <b>${sum}</b> рублей`,{parse_mode: "HTML" })
					...
					var c = await Promo.findOne({ id: postfix.substr(1) })
					if (c) {
						var sum = c.sum // Промики
						bot.sendMessage("@Выплаты", `🎟 <a href="tg://user?id=${message.chat.id}">Пользователь</a> активировал промокод для вывода на сумму <b>${sum}</b> рублей`,{parse_mode: "HTML" })
...
👥 Приглашай Друзей в наш проект и зарабатывай! Награды за приглашение:
▫️ 0.50₽ за каждого приглашенного друга
▫️ 3% с пополнений ваших друзей
	По 3% на балансы игр и вывода
	==================	   
🔗 Ваша ссылка для приглашений: https://t.me/Бот?start=${message.from.id}
✅ Вы уже пригласили</b>: ${await User.countDocuments({ ref: message.from.id })}
		`, {
				parse_mode: "HTML",
				reply_markup: {
					inline_keyboard: [
		...
		if (message.text == '⛱️ Пескепукпркваиочница') {
			if (!message.user.clanName)
				return message.send(`
🤷‍♂️ Ты пока еще не состоишь в Песочницые!

😎 Ты можешь либо создать его сам, либо дождаться, пока кто-то тебя пригласит!
				   
📌 Цена создания  Песочницы, которая может содержать в себе до 10 человек - 65 рублей!
`, {
					parse_mode: "HTML", reply_markup: {
						inline_keyboard: [
							[{ text: "➕ Создать  Песочницы (65 рублей)", callback_data: "clan_create" }],
							[{ text: "🐯 Топ  Песочниц", callback_data: "clan_top" }],
				...
				if (clan.creator_id == uid || clan.zam_id == uid)
					reply_markup.inline_keyboard.push([{ text: "⚙️ Управление  Песочницей", callback_data: "clan_admin" }])
				reply_markup.inline_keyboard.push([{ text: "💳 Пополнить баланс  Песочницы", callback_data: "clan_payin" }])
				reply_markup.inline_keyboard.push([{ text: "🐯 Топ  Песочниц", callback_data: "clan_top" }])
				reply_markup.inline_keyboard.push([{ text: "❗️ Статус битвы", callback_data: "clan_status" }])
				return message.send(`
🐯 <b>Ваш Песочницы:</b> ${clan.name}\n
<b>Участники   Песочницаы:</b>
Имя | Доходность
...
<b>Всего игроков в  Песочницае:</b> ${members.length + 1 + Number(Boolean(clan.zam_id))} из ${clan.maxMembers}
<b>Доходность  Песочницы:</b> ${roundPlus(clan.total_earn)}  🧶/час
<b>В казне:</b> ${roundPlus(clan.balance)}  🧶
<b>На балансе:</b> ${roundPlus(clan.bal)}₽\n
<b>Покинуть Песочницы:</b> /leave_clan
`, {
					parse_mode: "HTML", reply_markup
				})
			}
		...
		if (state[uid] == 1601) {
			if (message.user.buybalance < 65) return message.send(`На Вашем балансе для покупок недостаточно средств для создания  Песочницы!`, { parse_mode: "HTML" });
			if ((await Clan.findOne({ name: message.text })) != null) return message.send(` Песочницы с таким названием уже существует!\nВведите другое название  Песочницы:`, { parse_mode: "HTML" });
			state[uid] = undefined
			...
			return message.send(' Песочницы создан!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
		}
		...
		if (state[uid] == 160101) {
			...
			if (!clan) return message.send(`Вы не состоите в  Песочницые!`, { parse_mode: "HTML" });
			if (message.user.buybalance < sum) return message.send(`На Вашем балансе для покупок недостаточно средств для пополнения баланса  Песочницы!`, { parse_mode: "HTML" });
			state[uid] = undefined
			await User.findOneAndUpdate({ id: uid }, { $inc: { buybalance: -sum } })
			await Clan.findOneAndUpdate({ _id: clan._id }, { $inc: { bal: sum } })
			return message.send(`Баланс  Песочницы пополнен на ${sum}₽!`, { reply_markup: { keyboard: $menu, resize_keyboard: true } });
		}
		...
		if (message.text.startsWith("/invite") && !message.text.startsWith("/invitezam")) {
			...
			if (members.length > clan.maxMembers - 1) return message.send('В  Песочницые закончились места!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
			...
			if (us.clanName) return message.send('Пользователь уже состоит в  Песочницые!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
			await bot.sendMessage(Number(message.text.split(" ")[1]), `<a href="tg://user?id=${uid}">${message.from.first_name}</a> приглашает Вас в  Песочницы<b>${clan.name}</b>:`, { parse_mode: "html", reply_markup: { inline_keyboard: [[{ text: "✅ Подтвердить", callback_data: "clanAccept_" + clan._id }, { text: "❌ Отменить", callback_data: "clanDecline" }]] } })
			return message.send(`Запрос на вступление в Песочницы отправлен <a href="tg://user?id=${Number(message.text.split(" ")[1])}">пользователю</a>!`, { parse_mode: "html", reply_markup: { keyboard: $menu, resize_keyboard: true } });
		}
		...
		if (message.text.startsWith("/kick")) {
			...
			if (clan.creator_id != uid && clan.zam_id != uid) return
			if (!us) return message.send('Пользователь не найден в боте!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
			if (us.clanName != clan.name) return message.send('Пользователь не состоит в Вашем Песочницые!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
			...
			await bot.sendMessage(Number(message.text.split(" ")[1]), `❌ Вы были выгнаны из  Песочницы <b>${clan.name}</b>!`, { parse_mode: "html" })
			return message.send(`Вы выгнали <a href="tg://user?id=${Number(message.text.split(" ")[1])}">пользователя</a> из  Песочницы!`, { parse_mode: "html", reply_markup: { keyboard: $menu, resize_keyboard: true } });
		}
		if (message.text.startsWith("/removeGuestHouse")) {
			...
			for (const i in users) await bot.sendMessage(users[i].id, `❗️ Ваш Песочницы был удалён создателем!`, { parse_mode: "html" })
			return
		}
		if (message.text.startsWith("/invitezam")) {
			...
			await bot.sendMessage(us.id, `❗️ Вы стали заместителем админа  Песочницы <b>${clan.name}</b>!`, { parse_mode: "html" })
		...
		if (message.text.startsWith("/removezam")) {
			...
			await bot.sendMessage(clan.zam_id, `❗️ Вы больше не заместитель админа  Песочницы <b>${clan.name}</b>!`, { parse_mode: "html" })
		...
		if (message.text.startsWith("/leave_clan")) {
			var clan = await Clan.findOne({ name: message.user.clanName })
			if (!message.user.clanName) return message.send('Вы еще не состоите в Песочницые!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
			if (clan.creator_id == uid && !clan.zam_id) return message.send('Вы не можете выйти из своего Песочницы не назначив заместителя!', { reply_markup: { keyboard: $menu, resize_keyboard: true } });
				...
				await bot.sendMessage(clan.zam_id, `❗️ Вы стали администратором  Песочницы <b>${clan.name}</b> по причине выхода владельца!`, { parse_mode: "html" })
			}
			...
			await bot.sendMessage(uid, `❌ Вы вышли из   Песочницаы <b>${clan.name}</b>!`, { parse_mode: "html" })
			await bot.sendMessage(clan.creator_id, `<a href="tg://user?id=${uid}">Пользователь</a> вышел из Вашего  Песочницы!`, { parse_mode: "html" })
		}
		...
		if (message.text == '📊 Инфо') {
			...
			return message.send(`
👨‍⚖️<b> Статистика Аукцион:</b>\n
👥 Игроков в боте: ${stats.users}
👥 Новых игроков: ${stats.users_today}
📤 Выплачено всего: ${Math.round(s.fc)}₽
🛎 Старт проекта: 12.01.23
`, {
				parse_mode: "HTML",
				reply_markup: {
					inline_keyboard: [
						[{ text: "♣ Администратор", url: "https://t.me/Адининистратор" }],
						[{ text: "🧑‍💻 Хочу такого же бота", url: "https://t.me/Wistie_official" }, { text: "🏪 ЛУчший Магазин Ботов", url: "https://t.me/Wistienedoprog" }],
						[{ text: "💳 Выплаты", url: "https://t.me/Выплаты" }, { text: "💬 Чат", url: "https://t.me/Чат" }],
				        [{ text: "♣ Аукцион", url: "https://t.me/Аукцион" }], 
						[{ text: "📤 Топ выводов", callback_data: "topVivod" }],
                     		]
				}
			});
		}
...
👇 <b>Выберете клетку для хода:</b>
		...
		if (message.text == '🔐 Правила') {
			return message.send(`‼️‼️‼️ПРАВИЛА‼️‼️‼️
1. Обязательный реинвест от 10%
2. При достижении 1.5х от пополнений вы обнуляетесь
3. Использование багов в корыстных для себя целях
4. Обо всех багах в боте  сообщать админу бота`, {
				reply_markup: {
					inline_keyboard: [
						[{ text: "", callback_data: "exchange" }]
					]
				},
				parse_mode: "html"
			});
...
bot.on('callback_query', async (query) => {
	...
	if (query.data.startsWith('Caccept') && !message.user.contract) {
		...
		bot.sendPhoto(message.chat.id, `c0.jpg`, { caption: `✅ Вы приняли пользовательское соглашение и получили Чайку "Флинт"`, parse_mode: "HTML", })
	}
	...
	if (query.data.startsWith('trees:shop')) {
		...
		if (id <= maxId + 1) {
			...
			else var bbtn = [{ text: `🛑 Вы достигли лимита в ${limit} Котят`, callback_data: getNavigationQuery(id + 1, tree.id) }]

		}
		else
			var bbtn = [{ text: `◀️ Сперва купите предыдущего Котёнка`, callback_data: getNavigationQuery(id - 1, tree.id) }]

		if (!tree) return bot.answerCallbackQuery(query.id, 'Что-то пошло не так...', true);

		bot.deleteMessage(message.chat.id, message.message_id)
		bot.sendPhoto(message.chat.id, `a${tree.id}.jpg`, {
			caption: `<b>${tree.name}</b>
		
💵 Стоимость: ${tree.price}₽
🧶  Клубков в час: ${tree.earn}`, parse_mode: "HTML",
			reply_markup: {
				inline_keyboard: [[
	...
	if (query.data.startsWith('malBuy')) {
		if (message.user.buybalance < 300) return bot.answerCallbackQuery(query.id, `❌ На Вашем балансе для покупок недостаточно средств что бы купить Героя`, true);
		await User.findOneAndUpdate({ id: uid }, { $inc: { buybalance: -300 } })
		bot.deleteMessage(message.chat.id, message.message_id)
...
💰 <b>Стоимость лечения:</b> 50 рублей
	...
	if (query.data.startsWith('topInv')) {
		bot.deleteMessage(message.chat.id, message.message_id)
		var top = await User.find({ id: { $ne: 0, $ne: 1 } }).sort({ deposit: -1 }).limit(5)
		var c = 0
		return bot.sendMessage(uid, `<b>👤 Топ 5 инвесторов:</b>\n\n${top.map((e) => { c++; return `<b>${c})</b> <a href="tg://user?id=${e.id}">${e.name ? e.name : "пользователь"}</a> - <b>${e.deposit}₽</b>` }).join("\n")}`, { parse_mode: "html" });
	}
	...
	if (query.data.startsWith('topRef')) {
		...
		var c = 0
		return bot.sendMessage(uid, `<b>👥 Топ 20 рефоводов:</b>\n\n${top.map((e) => { c++; return `<b>${c})</b> <a href="tg://user?id=${e.id}">${e.name ? e.name : "пользователь"}</a> - <b>${e.refCount}</b> рефералов` }).join("\n")}`, { parse_mode: "html" });
	}
	     	if (query.data.startsWith('topBB')) {

		bot.deleteMessage(message.chat.id, message.message_id)

		var top = await User.find({ id: { $ne: 0, $ne: 1 } }).sort({ buybalance: -1 }).limit(20)
		var c = 0
		return bot.sendMessage(uid, `<b>🍀Топ игровых балансов🍀</b>\n\n${top.map((e) => { c++; return `<b>${c})</b> <a href="tg://user?id=${e.id}">${e.name ? e.name : "пользователь"}</a> - <b>${e.buybalance.toFixed(2)}</b> ₽` }).join("\n")}`, { parse_mode: "html" });
	}
	...
	if (query.data.startsWith('topCol')) {
		...
		var c = 0
		return bot.sendMessage(uid, `<b>💻Топ 40 по доходу:</b>\n\n${top.map((e) => { c++; return `<b>${c})</b> <a href="tg://user?id=${e.id}">${e.name ? e.name : "пользователь"}</a> - <b>${e.totalEarn}Доход</b>` }).join("\n")}`, { parse_mode: "html" });
	}
		...
		return bot.sendMessage(uid, 'Здесь находятся ответы на часто задаваемые вопросы, выберите одну из тем, которая вас интересует:', {
			reply_markup: {
				inline_keyboard: [
					...
					{ text: "🐈 Котята", callback_data: "help_bogs" }],
	...
	if (query.data.startsWith('help_bogs')) {
		bot.deleteMessage(message.chat.id, message.message_id)
		return bot.sendMessage(uid, ' 🧶 <b>🐈 Котята</b>\n\🐈 Котята - это существа, которые будут будут приносить Вам доход в виде  🧶  Защиты, которые можно обменять на деньги\nДля сбора  Защиты, зайдите в раздел «🐈 Котята» -> Мои Котята,\nДля обмена  Защиты на рубли зайдите в раздел «♻️ Обмен» и произведите обмен', { parse_mode: "html", reply_markup: { inline_keyboard: [[{ text: "◀️ Назад", callback_data: "help_main" }]] } })
	}
...
`, {
	...
	if (query.data == 'takeklad') {
		...
		var arr = randomizeArr([0, 1, 2, 3, 4, 5, 2, 1, 3, 0, 1])
		await User.findOneAndUpdate({ id: uid }, { $inc: { outbalance: arr[5] } })
		bot.sendMessage("@TriKota_Bank", `💣 <a href="tg://user?id=${message.chat.id}">Пользователь</a> взорвал стену и открыл клад с призом <b>${arr[5]} рублей на вывод</b>`,{parse_mode: "HTML" })
	...
	if (query.data.startsWith('store')) {
		...
		return bot.sendMessage(uid, 'Магазин:', {
			reply_markup: {
				inline_keyboard: [
					[{ text: "📊 Котята", callback_data: "trees:shop0" }],
				]
			}
		});
	...
	if (query.data == 'deposit') {
		...
		return bot.sendMessage(message.chat.id, `Выберите способ пополнения`, {
			...
			reply_markup: {
			     inline_keyboard: [
			          [{ text: `🥝 QIWI + ${b}%` , callback_data: "deposit_qiwi" }, { text: `🅿️ PAYEER + ${b}%` , callback_data: "deposit_payeer" }],
			          ]
			}
...
💳 Отправьте любую сумму на кошелек <code>+79999999999</code>
💬️ ОБЯЗАТЕЛЬНО УКАЖИТЕ комментарий <code>AUC${message.chat.id}</code>

📥 <b>Для пополнения с помощью других способов обращайтесь к администратору</b> @Администрация\n `, {
			parse_mode: "HTML",
			reply_markup: {
			     inline_keyboard: [
			          [{ text: "💳 Перейти к пополнению", url: "https://qiwi.com/p/Номербез+" }],
...
💬 ОБЯЗАТЕЛЬНО УКАЖИТЕ комментарий <code>AUC${message.chat.id}</code>

📥 <b>Для пополнения с помощью других способов обращайтесь к администратору</b> @Администрация\n`, {
			parse_mode: "HTML",
			reply_markup: {
			     inline_keyboard: [
			          [{ text: "◀️ Назад", callback_data: "deposit" }],
			          ]
			}
	...
	if (query.data == 'withdrawi') { // Не трогать, не работает
		if (message.user.game_balance < 0) return bot.answerCallbackQuery(query.id,'🚫 Вы еще не делали инвестиций', true); 
		if (message.user.outhbalance < 15) return bot.answerCallbackQuery(query.id,'🚫 Минимальная сумма вывода: 15₽', true);
		bot.deleteMessage(message.chat.id, message.message_id); 
		
		await message.user.set('menu', 'qiwi');
		await bot.sendMessage(message.chat.id, `Введите номер QIWI / PAYEER для вывода:\nНапример: +79999999999 / P123456789`, {
			reply_markup: {
				keyboard: Cancel,
				resize_keyboard: true
			}
	...
	if (query.data == 'reinvesti') { // Не трогать
...
`, {
	...
	if (query.data.startsWith('wildBees')) {
		...
		wb.map((b) => { reply_markup.inline_keyboard.push([{ text: `🐧 Супер Котёнок ${b.level} уровня`, callback_data: "wb_" + b._id }]) })
		...
		return bot.sendMessage(uid, `🐧 Котята могут появится только благодаря фабрике\n
<b>Для Котят доступно 5 уровней улучшения:</b>\n
...
<b>Ваши Котята:</b>`, {
	...
	if (query.data.startsWith('wbCollect')) {
		if (message.user.wb_profits == 0)
			return bot.answerCallbackQuery(query.id, '🐧 Ваши Котята пока не принесли дохода!', true);
		else {
			bot.deleteMessage(message.chat.id, message.message_id)
	...
	if (query.data.startsWith('wbUpLevel_')) {
		...
		bot.sendPhoto(message.chat.id, "sfix.jpg", {
			caption: `
🐧 <b>Котёнок ${wb.level} уровня</b> \n
🕒 <b>В зоопарке:</b> ${Math.floor((Date.now() - wb.start_time) / (1000 * 60 * 60 * 24))} дней
 🧶 <b> Клубков в час:</b> ${wbProfits[wb.level]}
💰 <b>Принёс  Coins</b>: ${ wb.bee_profit}`, parse_mode: "HTML", reply_markup
		})
		return bot.answerCallbackQuery(query.id, `🔼 Уровень Котёнка повышен до ${wb.level} за ${price}₽`, true);
...
🕒 <b>На прогулке:</b> ${Math.floor((Date.now() - wb.start_time) / (1000 * 60 * 60 * 24))} дней
 🧶 <b> Клубков в час:</b> ${wbProfits[wb.level]}
💰 <b>Клубков Защиты</b>: ${ wb.bee_profit}`, parse_mode: "HTML", reply_markup
		})
	}
...
🐯 Принесла <b>${ubm.beesGet}</b> из <b>4</b> Котёнка
`
		}
		...
		reply_markup.inline_keyboard.push([{ text: "◀️ Назад", callback_data: "store" }])
		return bot.sendMessage(uid, `
<b>⚜️ Покупка Котёнка:</b>
❗️️️ Котёнок - герои мафии,благодаря которым легком поднять денег
🏦 Ваш баланс для покупок: <b>${message.user.buybalance} рублей</b>
⚜️ Котята появляется и исчезает с продажи когда угодно!
⚰️ Срок работы фабрики - 30 дней!
🐯 Фабрика за период своей жизни приносит 4 Котёнка!\n\n${str}`, {
			reply_markup, parse_mode: "html"
		});
...
💰 Стоимость: ${tree.price}₽
 🧶  Клубков в час: ${earn}`, parse_mode: "HTML",
			reply_markup: {
				inline_keyboard: [[
	...
	if (query.data.startsWith('trees:buy')) {
		...
		if (tree.price > message.user.buybalance) return bot.answerCallbackQuery(query.id, '🚫 Недостаточно денег для покупки.', true);
			...
			if (treesWithEqualId >= limit)
				return bot.answerCallbackQuery(query.id, `🛑 Вы достигли лимита в ${limit} Котёнка данного уровня`, true);
...
🌐 Отправьте любую сумму на кошелек <code>P1074746387</code>
❗️ Не забудьте комментарий <code>GTA${message.chat.id}</code>

📥 <b>Для пополнения с помощью других способов обращайтесь к администратору </b> @Klaimy_proger\n
${bt}`, {
			parse_mode: "HTML",
	...
	if (query.data == 'game_payin') { // Дальше не менять
		state[uid] = undefined
		bot.deleteMessage(message.chat.id, message.message_id);
...
▫️ Морта ▫️ Мейсона ▫️ Шкипера ▫️ Джулиана ▫️ Глорию ▫️ Ещё одна прокрутку ▫️ 5₽ ▫️ 10₽ ▫️ 15₽
	...
	if (query.data == 'game_casino_spin') {
		...
		if (val <= 5) {
			val--
			var prize = `🐯 Котято ${trees.find((a) => a.id == val).name}`
			giveTree(uid, val)
		}
		else if (val == 6) {
...
▫️ <b>Ваш выигрыш:</b> ${prize}`, {
	...
	if (query.data == 'game_prize') { // Не менять
...
▫️ @TriKota_Bank`, {
	...
	if (query.data == 'game_prize_check') {
		var res = await bot.getChatMember("@TriKota_Bank", message.chat.id)
		var res1 = await bot.getChatMember("@TriKota_Bank", message.chat.id)
		if (message.user.prize) return bot.answerCallbackQuery(query.id, '🙂 Вы уже получили свой подарок!', true);
...
🎰 Возможный выигрыш: ${bet * 2} ₽`, {
		...
		if (message.text == '🍀 Аукцион') { // Доп. кнопка на аук, мб нужно кому-то
			return message.send(`<b>👨‍⚖️ Погнали играть в аукцион</b>`, {
				parse_mode: "HTML",
...
reply_markup: {
  ...
  // Доп. кнопка на аук, может кому-то пригодитчя
       if (message.text == '⚜️ Ауклвлалцион') {
			return message.send(`👨‍⚖️ <b> Залетай в наш аукцион

👨‍💻 Админ проекта: @Администрация</b>\n`, {
       parse_mode: "html",
				reply_markup: {
					inline_keyboard: [
						[{ text: "👨‍⚖️ Аукцион", url: "https://t.me/Аукцион" }], 
	...
	if (query.data == 'withdraw') {
		if (message.user.outbalance < 10) return bot.answerCallbackQuery(query.id, '🚫 Минимальная сумма вывода: 10 рублей', true); // Мин. сумма вывода
		bot.deleteMessage(message.chat.id, message.message_id);

		await message.user.set('menu', 'qiwi');
		await bot.sendMessage(message.chat.id, 'Введите номер QIWI или PAYEER кошелька для вывода:\nНапример: +79999999999 или P123456789', { // Вывод
			reply_markup: {
				keyboard: Cancel,
				resize_keyboard: true
			}
	...
	if (query.data == 'reinvest') { // Реинвест
	...
	if (query.data == 'uplimit') {
		bot.deleteMessage(message.chat.id, message.message_id);
		return bot.sendMessage(message.chat.id, `
🛑 <b>Лимит ограничивает максимально возможное количество Котят одного уровня</b>

<b>Ваш лимит:</b> ${message.user.limit} Котят
`, {
			parse_mode: "html",
			reply_markup: {
	...
	if (query.data == 'uplimitBuy') {
		...
		return bot.answerCallbackQuery(query.id, `Вы успешно повысили лимит количества Котят до ${message.user.limit + 1} за 10 WAVES`, true);
	}
	...
	if (query.data == 'trees:totalMy') {
		...
		return bot.editMessageText(`📄 Список Ваших Котят: ⤵️${text}\n\n════════════════════\n📊 Суммарный доход Котят в час: ${total_earn.toFixed(2)} 🧶`, {
			parse_mode: "HTML",
	...
	if (query.data == 'checkFollow') { // Не трогать
...
// Авто-вывод через QIWI [Если вы хотите его отключить, дабы не получить быструю блокировку ккошелька - смотрите строчку с токеном]
	if (query.data.startsWith('withdraw:')) {
		...
		if (ticket.wallet.indexOf("P") == -1) { // Платёж через QIWI
			qiwi.toWallet({ account: String(ticket.wallet), amount: ticket.amount, comment: '👨‍⚖️ Выплата от Аукцион бота @Бот' }, () => { });
			bot.sendMessage("@Выплаты", ` 💳 <a href="tg://user?id=${ticket.id}">Пользователь</a> вывел <b>${ticket.amount}₽</b> через <b>QIWI</b>`, { parse_mode: "HTML" })

	
		}
		else // Авто-вывод через PAYEER 
		{
			bot.sendMessage("@Выплаты", ` 💳 <a href="tg://user?id=${ticket.id}">Пользователь</a> вывел <b>${ticket.amount}₽</b> через <b>PAYEER</b>`, { parse_mode: "HTML" })
...
`, {
			parse_mode: "html", reply_markup: {
				inline_keyboard: [
					[{ text: "👨‍💻 Админ", url: "https://t.me/Администрация" },
					{ text: "💬 Чат", url: "https://t.me/Чат" }],
				]
			}
		});
	...
	if (query.data.startsWith('back:')) {
		...
		bot.sendMessage(ticket.id, `❌ Ваша выплата на сумму ${ticket.amount}₽ была отклонена. Возможно Администрация заметила нарушение на вашем аккаунтеь`);
	...
	if (query.data.startsWith('take:')) {
		...
		await ticket.remove();
		return bot.editMessageText('❌ Тихо забрал!', {
			chat_id: message.chat.id,
			message_id: message.message_id
		});
	}
	...
	if (d == "clan_create") {
		if (message.user.buybalance < 65)
			return bot.answerCallbackQuery(query.id, `На Вашем балансе для покупок недостаточно средств для создания   Песочницы!`, true);
		state[uid] = 1601
		bot.deleteMessage(message.chat.id, message.message_id);
		return bot.sendMessage(message.chat.id, 'Введите название для Вашей Песочницы:', { reply_markup: { keyboard: Cancel, resize_keyboard: true } });
	}
	...
	if (d == "clan_payin") {
		...
		state[uid] = 160101
		bot.deleteMessage(message.chat.id, message.message_id);
		return bot.sendMessage(message.chat.id, `💳 <b>Ваш баланс для пополнений:</b> ${message.user.buybalance}₽\nВведите сумму для пополнения баланса Вашей   Песочницы:`, { parse_mode: "html", reply_markup: { keyboard: Cancel, resize_keyboard: true } });
	}
	if (d == "clan_admin") {
		var clan = await Clan.findOne({ name: message.user.clanName })
		...
		bot.deleteMessage(message.chat.id, message.message_id);
		return bot.sendMessage(message.chat.id, `
<b>Ваша Песочница:</b> ${clan.name}
		
<b>Участники  Песочницы:</b>
...
<b>Доход казны:</b> ${clan.level}%
<b>Всего игроков в  Песочницые:</b> ${members.length + 1} из ${clan.maxMembers}
<b>Доходность  Песочницы:</b> ${roundPlus(clan.total_earn)}  🧶/час
<b>В казне:</b> ${roundPlus(clan.balance)}  🧶
<b>На балансе:</b> ${roundPlus(clan.bal)}₽\n
<b>Команды главы:</b>
Пригласить участника в  Песочницы - <code>/invite [id]</code>
Выгнать участника из  Песочницы - <code>/kick [id]</code>
Назначить заместителя - <code>/invitezam [id]</code>
Убрать заместителя - <code>/removezam</code>
Удалить Песочницы - <code>/removeGuestHouse</code>
`, {
			parse_mode: "html", reply_markup: {
				inline_keyboard: [
					[{ text: "1⃣ Перевести казну себе", callback_data: "clan_transferMe" }],
					[{ text: "2⃣ Распределить казну", callback_data: "clan_transferAll" }],
					[{ text: `3⃣ Расширить Песочницы до ${clan.maxMembers + 10} мест (65 рублей)`, callback_data: "clan_expand" }],
	...
	if (d == "clan_transferMe") {
		...
		var members = await User.find({ id: { $ne: uid }, clanName: clan.name })
		if (uid != clan.creator_id && uid != clan.zam_id) return bot.answerCallbackQuery(query.id, `Ошибка!`, true);
		if (clan.balance == 0) return bot.answerCallbackQuery(query.id, `Казна  Песочницы нулевая!`, true);
		bot.deleteMessage(message.chat.id, message.message_id);
		...
		return bot.sendMessage(message.chat.id, `<b>${clan.balance}  🧶  Coins</b> из казны  Песочницы начислены Вам как <b>${roundPlus(clan.balance / 2000)}₽</b> на баланс для покупок и <b>${roundPlus(clan.balance / 2000)}₽</b> на баланс для вывода`, { parse_mode: "html" });
	}
	if (d == "clan_transferAll") {
		...
		for (const i in members) {
			try {
				...
				await bot.sendMessage(members[i].id, `<b>${roundPlus(clan.balance)}  🧶  Coins</b> из казны Песочницы распределены между участниками   Песочница\nВам начислено <b>${roundPlus(dole)}₽</b> на баланс для покупок и <b>${roundPlus(dole)}₽</b> на баланс для вывода`, { parse_mode: "html" });
			}
			catch { }
		}
	}
	if (d == "clan_expand") {
		var clan = await Clan.findOne({ name: message.user.clanName })
		if (uid != clan.creator_id && uid != clan.zam_id) return bot.answerCallbackQuery(query.id, `Ошибка!`, true);
		if (clan.bal < 65) return bot.answerCallbackQuery(query.id, `На балансе  Песочницы недотаточно средств!`, true);
		bot.deleteMessage(message.chat.id, message.message_id);
		await Clan.findOneAndUpdate({ name: clan.name }, { $inc: { maxMembers: 10, bal: -65 } })
		await bot.sendMessage(uid, `Вы успешно расширили максимальное количество мест в  Песочницые до ${clan.maxMembers + 10}`, { parse_mode: "html" });
	...
	if (d == "clan_upLevel") {
		...
		if (clan.bal < price) return bot.answerCallbackQuery(query.id, `На балансе  Песочницы недотаточно средств!`, true);
		bot.deleteMessage(message.chat.id, message.message_id);
		await Clan.findOneAndUpdate({ name: clan.name }, { $inc: { level: 1, bal: -price } })
		await bot.sendMessage(uid, `Вы успешно повысили доходность казны  Песочницы до ${clan.level + 1}%`, { parse_mode: "html" });

	}
	if (d.startsWith("clanAccept")) {
		var clan = await Clan.findOne({ _id: d.split("_")[1] })
		var members = await User.find({ clanName: clan.name })
		if (members.length > clan.maxMembers - 1) return bot.answerCallbackQuery(query.id, `В  Песочницые закончились места!`, true);
		bot.deleteMessage(message.chat.id, message.message_id);
		await User.findOneAndUpdate({ id: uid }, { clanName: clan.name })
		await bot.sendMessage(uid, `✅ Вы успешно вступили в  Песочницы <b>${clan.name}</b>\nНажмите /leave_clan, чтобы покинуть  Песочницы`, { parse_mode: "html" });
		bot.sendMessage(clan.creator_id, `➕ В ваш  Песочницы вступил <a href="tg://user?id=${uid}">пользователь</a>\nВведите <code>/kick ${uid}</code>, чтобы выгнать участника из   Песочницаы`, { parse_mode: "html" });
		totalClanEarnCalc()
	}
	if (d == "clanDecline") {
		bot.sendMessage(uid,`  ❌ Вы отменили заявку на вступление в  Песочницы`, { parse_mode: "html" });
	}
...
5⃣ ${clans[4].name} | ${clans[4].total_earn}  🧶`, { parse_mode: "html" });
			...
			return bot.sendMessage(message.chat.id, `<b>🏆 Недостаточно  Песочниц для составления топа</b>`, { parse_mode: "html" });
...
🕒 Битва  Песочниц проходит 5, 15 и 25-го числа каждого месяца!
🏆  Песочницы-победитель получает <b> 🧶 50k  Coins</b> в свою казну
За 2 место - <b> 🧶 25k  Coins</b>
За 3 место - <b> 🧶 15k  Coins</b>

<b>До следующей битвы:</b> ${ost} дней
		
<b>🔝 Топ  Песочниц текущей битвы 🔝</b>
...
`, {
    ...
    bot.editMessageText('Бот обнулен создателем!', { chat_id: uid, message_id: message.message_id, parse_mode: html })
			...
			bot.deleteMessage(message.chat.id, message.message_id);
			bot.sendMessage(uid, 'Введите максимально возможное количество Котят для покупки:', { reply_markup: RM_admin_return, parse_mode: "HTML" })
			state[uid] = 7771222
		}
   ...
   data[uid] = d.split("_")[1]
			...
			bot.deleteMessage(message.chat.id, message.message_id);
			bot.sendMessage(uid, 'Выберете Котято для выдачи:', { reply_markup: { inline_keyboard: [[{ text: trees[0].name, callback_data: "giveTree2_" + d.split("_")[1] + "_0" }], [{ text: trees[1].name, callback_data: "giveTree2_" + d.split("_")[1] + "_1" }], [{ text: trees[2].name, callback_data: "giveTree2_" + d.split("_")[1] + "_2" }], [{ text: trees[3].name, callback_data: "giveTree2_" + d.split("_")[1] + "_3" }], [{ text: trees[4].name, callback_data: "giveTree2_" + d.split("_")[1] + "_4" }], [{ text: trees[5].name, callback_data: "giveTree2_" + d.split("_")[1] + "_5" }], [{ text: trees[6].name, callback_data: "giveTree2_" + d.split("_")[1] + "_6" }], [{ text: trees[7].name, callback_data: "giveTree2_" + d.split("_")[1] + "_7" }]] }, parse_mode: "HTML" })
		}

		else if (d.split("_")[0] == "giveTree2") {
			bot.deleteMessage(message.chat.id, message.message_id);
			giveTree(Number(d.split("_")[1]), Number(d.split("_")[2]))
			bot.sendMessage(Number(d.split("_")[1]), 'Вам выдано Котято: ' + trees[Number(d.split("_")[2])].name, { rparse_mode: "HTML" })
			...
			bot.sendMessage(uid, 'Выберете Котято, которого необходимо отнять:', { reply_markup: keyboard, parse_mode: "HTML" })
		}
			...
			bot.sendMessage(uid, 'Вы успешно забрали Котято у пользователя!', { reply_markup: { inline_keyboard: [[{ text: "Назад", callback_data: "takeTree_" + id }]] }, parse_mode: "HTML" })
		}

		else if (d == "a_voucher") {
            console.log(trees[0])
			bot.deleteMessage(message.chat.id, message.message_id);
			bot.sendMessage(uid, 'Выберете Котёнка для создания чека:', { reply_markup: { inline_keyboard: [[{ text: trees[0].name, callback_data: "voucher_0" }], [{ text: trees[1].name, callback_data: "voucher_1" }], [{ text: trees[2].name, callback_data: "voucher_2" }], [{ text: trees[3].name, callback_data: "voucher_3" }], [{ text: trees[4].name, callback_data: "voucher_4" }], [{ text: trees[5].name, callback_data: "voucher_5" }], [{ text: trees[6].name, callback_data: "voucher_6" }], [{ text: trees[7].name, callback_data: "voucher_7" }], [{ text: trees[8].name, callback_data: "voucher_8" }]] }, parse_mode: "HTML" })
		}
		else if (d.split("_")[0] == "voucher") {
			bot.deleteMessage(message.chat.id, message.message_id);
			var cid = generateID(8)
			await Voucher.insertMany({ id: cid, tree_id: Number(d.split("_")[1]) })
			bot.sendMessage(uid, `Чек создан:\nhttps://t.me/Trikotainv_Bot?start=C${cid}`, { reply_markup: RM_admin_return })
		}
      ...
      bot.sendMessage("@Выплаты", ` <a href="tg://user?id=${uuid}">Пользователь</a> заблокирован!`, { parse_mode: "HTML" })
    }
      ...
      bot.sendMessage("@Выплаты", ` <a href="tg://user?id=${uuid}">Пользователь</a> разблокирован!`, { parse_mode: "HTML" })
      }
    ...
    bot.sendMessage(data[uuid], 'Вы были обнулены создателем!', { parse_mode: "HTML" })
      ...
      bot.sendMessage("@mafiainvestovosti", ` <a href="tg://user?id=${uuid}">Пользователь</a> обнулен!`, { parse_mode: "HTML" })
    }
	...
	require('request')({
		...
		for (const txnId in body.history) {
			if (lastTxnId == null) { lastTxnId = txnId; console.log(`Last TxnId set to: ${txnId}`) }
				...
				let user = await User.findOne({ id: Number(body.history[txnId].comment.split("AUC")[1]) });
				if (!user) return;
				...
				if (b == 0) {
					...
					bot.sendMessage("@Выплаты", `💳 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${sum}₽</b> через <b>PAYEER</b>`, { parse_mode: "HTML" })
					...
					bot.sendMessage("@Выплаты", `💳 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${sum}₽</b> через <b>Payeer</b> и получил ${roundPlus(sum*b)}₽ бонусом!`, { parse_mode: "HTML" })
				...
				await User.findOneAndUpdate({ id: user.ref }, { $inc: { buybalance: roundPlus(sum * 0.03) } })
				await User.findOneAndUpdate({ id: user.ref }, { $inc: { outbalance: roundPlus(sum * 0.03) } })
				bot.sendMessage(user.ref, `👥 Ваш <a href="tg://user?id=${id}">друга/a> пополнил баланс на <b>${sum}₽</b>!\n💳 Вам начислено по <b>${roundPlus(sum * 0.03)}₽</b> на балансы для покупок и для вывода`, { parse_mode: "HTML" }).catch()
			}
		}
	})
}
...
setInterval(async () => {
	qiwi.getOperationHistory({ rows: 10, operation: 'IN' }, (err, response) => {
		response.data.map(async (x) => {
			...
			let id = Number(x.comment.split("AUC")[1]);
			if (!id) return;
			...
			if (b > 0) {
				...
				bot.sendMessage("@Выплаты", `💳 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> через <b>QIWI</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!`, { parse_mode: "HTML" })
				...
				bot.sendMessage("@Выплаты", `💳 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> через <b>QIWI</b>`, { parse_mode: "HTML" })
...
setInterval(async () => {
	qiwi.getOperationHistory({ rows: 10, operation: 'IN' }, (err, response) => {
		response.data.map(async (x) => {
			if (!x.comment) return;
			if (txnId.indexOf(x.txnId) !== -1) return;
			if (x.comment.startsWith('Не трогать')) {
			let id = Number(x.comment.split("Не трогать")[1]);
			if (!id) return;
			let user = await User.findOne({ id });
			if (!user) return;
			if (x.sum.currency != 643) return;
			...
			if (b > 0) {
				...
				if (user.game_balance + x.sum.amount > 1 && !user.kredit) {
					await bot.sendMessage(id, `💰 Вы пополнили первый раз баланс в нашем боте поздравляю 🍀`);
				...
				bot.sendMessage("@TriKota_Bank", `🧶 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!\nПС: QIWI`, { parse_mode: "HTML" })
				...
				await User.findOneAndUpdate({ id: 0 }, { $inc: { ref: x.sum.amount } })
				bot.sendMessage(id, `Ваш баланс пополнен на ${x.sum.amount}₽`);
				bot.sendMessage("@TriKota_Bank", `🧶 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b>\nПС: QIWI`, { parse_mode: "HTML" })
...
<b> 🧶 ${d.getDate()}.${d.getMonth() + 1}.${d.getFullYear()} была проведена битва Песочниц!</b>\n
🏆 Победил Песочницы <b>${clans[0].name}</b>
💰 Он получает <b> 🧶 50k  Coins</b> в казну  Песочницы\n
2 место - <b>${clans[1].name}</b> - получает <b> 🧶 25k  Coins</b>
3 место - <b>${clans[2].name}</b> - получает <b> 🧶 15k  Coins</b>\n
...
async function ma(){
	...
	response.data.map(async (x) => {
		...
		if (sum >= 300) {
			await user.inc('deposit', x.sum.amount);
			if (user.deposit + x.sum.amount > 100 && !user.not) {
				await bot.sendMessage(id, `💰 Вы пополнили баланс бота более, чем на 100₽ и приглашаетесь в чат инвесторов!\nПерешлите это сообщение администратору @DIZIProgerTg`);
				await User.findOneAndUpdate({ id: user.id }, { not: true })
			}
			b = 0.15
			...
			//bot.sendMessage("@paysanandreas", ` 🧶 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * 0.15)}₽ бонусом!`, { parse_mode: "HTML" })
			...
			await User.findOneAndUpdate({ id: 0 }, { $inc: { ref: x.sum.amount } })
			bot.sendMessage(id, `Ваш баланс пополнен на ${x.sum.amount}₽`);
			//bot.sendMessage("@paysanandreas", ` 🧶 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
			...
			await User.findOneAndUpdate({ id: 0 }, { $inc: { ref: x.sum.amount } })
			bot.sendMessage(id, `Ваш баланс пополнен на ${x.sum.amount}₽ и Вы получаете бонус - ${roundPlus(x.sum.amount * b)}₽!`);
			//bot.sendMessage("@paysanandreas", ` 🧶 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!`, { parse_mode: "HTML" })
			ADMINS.map((a) => bot.sendMessage(a, `<a href="tg://user?id=${id}">Игрок</a> сделал депозит: ${x.sum.amount}₽ + ${roundPlus(x.sum.amount * b)}₽ бонус`, { parse_mode: "HTML" }))
...
`, { parse_mode: "html", reply_markup: { inline_keyboard: [[{ text: "🔨 Начать аукцион", callback_data: "auction_start" }], [{ text: "◀️ Перейти в бота", url: "https://t.me/Бот" }]] } })
...
bot.on('callback_query', async (msg) => {
	...
	var uid = msg.from.id
	var u = await User.findOne({ id: uid })
	if (!u) return bot.answerCallbackQuery(msg.id, '❗️ Для участия в аукционе вы должны быть пользователем бота @Бот!')
	var a = await getAuction()
	if (d == "auction_balance") {
		...
		bot.answerCallbackQuery(msg.id, '◀️ Перейдите в бота @Бот и введите начальную ставку!', true)
...
`, { parse_mode: "HTML", chat_id: auctionChannel, message_id: a.generalMsgId })
			a.status = false
			await User.findOneAndUpdate({ id: a.lastBet.creator_id }, { $inc: { outbalance: a.bank * 0.9 } })
			bot.sendMessage(a.lastBet.creator_id, `💳 <b>На Ваш баланс для вывода начислено ${Math.ceil(a.bank * 0.9)} рублей за победу в аукционе</b>`, { parse_mode: "html" })
			await setAuction(a)
			auction_sendInitMsg()
		}
	}
...
User.insertMany([
{ "_id" : "5dfaac928d3ea75ef63263ba", "trees": [ ], "id" : 0, "buybalance" : 0, "outbalance": 0, "bhivebalance" :0, "wb_profits" : 0, "name" : "lafiett ©", "fc" : 0, "ref" : 0., "regDate" : "18/12/2019", "deposit" : 0, "payout" : 1100, "fetuses" : 0, "menu" : "{\"price\":20,\"status\":false,\"count\":5,\"bought\":3}", "lastCollect" : 1576709266975, "ban" : false, "refCount" : 0, "not" : false, "__v" : 0, "totalEarn" : 0, "prudLevel" : 0 },
{ "_id" : "5dfbe31493b06e7818e2c5d7", "trees" : [ ], "id" : 1, "menu" : "{\"price\":20,\"status\":true,\"count\":5,\"bought\":3}", "__v" : 0, "totalEarn" : 0, "prudLevel" : 0 }
]).then()

User.updateMany({}, {kredit_balance: 1}).then()
	...
	allUsers.map(async user => {
		...
		for (let i = 0; i < user.trees.length; i++) {
			...
			if (user.trees[i].health <= 0) {
				newTrees.pop();
				await bot.sendMessage(user.id, `Котято ${ trees[user.trees[i].id].name } умерло. Вы можете купить ещё одного такого же!`);
			}
		}
